...
class MainUI(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        #        self.robot = XYRobotGui.XYBot(self.scene, self.ui)

        #       self.robot.sendCmd = self.sendCmd
        #       self.robot.robotSig = self.robotSig

        # connect robot delegate
        self.ui.btnHome.clicked.connect(self.robotGui.robotGoHome)
    ...
    def portComboPressed(self, event):
        logger.info("portComboPressed")

        self.refreshCom()
        self.ui.portCombo.showPopup()

    def refreshDone(self, msg):
        logger.info("refreshDone")
    ...
    def loadPic(self, filename=False):
        ...
        if filename == False:
            filename = str(
                QtGui.QFileDialog.getOpenFileName(self, 'Open Svg/Bmp', '', ".svg;.bmp(*.svg;*.bmp)").toUtf8())
        self.dbg(filename)
        if len(filename) == 0:
            return
